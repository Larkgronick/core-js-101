...
function findFirstSingleChar(str) {
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (str.indexOf(curr) === i && str.indexOf(curr, i + 1) === -1) {
      return curr;
    }
  }
  return null;
...
function isCreditCardNumber(ccn) {
  const input = ccn.toString();
  let sum = 0;
  let double = false;
  for (let i = input.length - 1; i >= 0; i -= 1) {
    const curDigit = parseInt(input.charAt(i), 10);
    if (double) {
      if ((curDigit * 2) > 9) {
        sum += (curDigit * 2) - 9;
      } else {
...
function toNaryString(num, n) {
  const a = parseInt(num, 10).toString(n);
  return a;
}
...
function getCommonDirectoryPath(pathes) {
  const a1 = pathes[0];
  const a2 = pathes[pathes.length - 1];
  const L = a1.length;
  let i = 0;
  while (i < L && a1.charAt(i) === a2.charAt(i)) i += 1;
  const str = a1.substring(0, i);
  const boundary = str.lastIndexOf('/') + 1;
  const result = str.substring(0, boundary);
  return result;
